...
export const CORE_ACCOUNTS: AccountSeed[] = [
  {
    ...
    tickets: [
      { createdAt: "2026-06-24", subject: "Raise ingest rate limit to 60k events per second for the fraud pipeline", priority: "P2", status: "resolved", slaBreached: false, body: "Limit raised from 25k to 60k events per second on the Helios pilot tenant ahead of their load test. Their run sustained 58k events per second over four hours with no throttling and no consumer lag growth. Resolved same day." },
      { createdAt: "2026-06-26", subject: "Kafka source connector: confirming exactly-once semantics end to end", priority: "P3", status: "resolved", slaBreached: false, body: "Tom Feld asked how the connector behaves with their idempotent producer configuration and whether offsets are committed transactionally with the sink write. Confirmed transactional commit and shared the checkpoint configuration for their fraud pipeline. Tom validated it against a deliberate consumer restart and saw no duplicates." },
      { createdAt: "2026-07-13", subject: "Okta SCIM provisioning for the fraud analytics group", priority: "P3", status: "resolved", slaBreached: false, body: "Configured SCIM provisioning and group to role mapping for the incoming 40 person fraud analytics cohort. Deprovisioning tested against a suspended test user and confirmed working within one sync cycle." },
    ],
    calls: [
      { callId: "GONG-104", callDate: "2026-06-29", title: "Benchmark readout and reference architecture review", participants: "Sofia Vega (Director of Data Platform), Tom Feld (Data Engineering Lead), Yusuf Abdel-Rahman", transcript: "Tom presented the results of the load test his team ran themselves: 58k events per second sustained, p95 at 210ms and p99 at 380ms against their internal target of 500ms. We walked the dual region active-passive reference architecture and Sofia signed it off on the call, with the only change being a shorter replication lag alarm threshold on their side. Tom mentioned in passing that the payments platform group has started asking his team about the same connector, though nothing has been scoped.", committedNextStep: "Vantage to send the streaming tier proposal and the reference architecture document by 2026-07-06", nextStepStatus: "delivered" },
      { callId: "GONG-118", callDate: "2026-07-15", title: "Proposal and security walkthrough", participants: "Aisha Bello (VP Engineering), Sofia Vega (Director of Data Platform), Devin Osei (Staff Security Engineer), Yusuf Abdel-Rahman", transcript: "Devin closed out both of his open questions on the call: customer-managed key support is confirmed for the streaming tier, and network egress stays inside their VPC over PrivateLink. He had no further findings and said he would put that in writing after a formal review session. Aisha said the budget is not a concern and that she will walk it through procurement, and she asked for the security session before anything is signed.", committedNextStep: "Security review call on 2026-07-22", nextStepStatus: "none" },
    ],
    usage: [
...

  {
    ...
    tickets: [
      { createdAt: "2026-06-11", subject: "CDC connector drops rows during Postgres primary failover", priority: "P1", status: "escalated", slaBreached: true, body: "During Northwind's own failover drill the logical replication slot was not preserved on promotion of the standby, and the connector resumed from a stale LSN. Ines Duarte reconciled 41,900 missing rows across three fact tables. Phase 2 exit criterion 3 requires zero row loss across a controlled failover, so this run is recorded as a fail. Engineering has a fix targeted but no build date has been given to the customer." },
      { createdAt: "2026-06-19", subject: "p99 query latency above target on the wide-fact workload", priority: "P2", status: "resolved", slaBreached: false, body: "Northwind measured p95 at 410ms and p99 at 1,240ms on their 92 column order-line fact table at 40k events per second. Resolved as configuration rather than defect: partition key and clustering guidance applied, which brought p99 down to 640ms on the customer's re-run. The phase 2 exit criterion is p99 under 500ms at 40k events per second, so the criterion remains unmet." },
      { createdAt: "2026-06-30", subject: "72 hour ingest soak test aborted at 42 hours, sustained backpressure", priority: "P1", status: "open", slaBreached: true, body: "The streaming tier soak test was aborted at hour 42 when consumer lag passed 900 seconds and did not recover. The ingest API call rate flattened near 9.4M per week while event throughput held near 40k per second. Marcus Rho's team asked whether the ceiling is a per tenant shard limit or their own broker configuration. Still open, no root cause issued." },
      { createdAt: "2026-07-06", subject: "Pen test finding: customer-managed keys not enforced on staged objects", priority: "P2", status: "open", slaBreached: false, body: "Northwind's third party pen test found that objects in the ingestion staging bucket are encrypted with platform-managed keys rather than the customer's KMS key. Priya Nandakumar's team logged it as a medium finding requiring remediation or a documented compensating control before production cutover. Key management design doc supplied on 2026-07-09, customer verification pending." },
      { createdAt: "2026-07-13", subject: "PrivateLink endpoint required, public ingest endpoint not permitted in prod", priority: "P2", status: "open", slaBreached: false, body: "Northwind's network standard forbids egress to public service endpoints from the production VPC. They need a PrivateLink endpoint in eu-central-1 plus a documented DNS pattern for their split-horizon resolver. Currently the pilot runs over an allow-listed NAT gateway that their platform team has confirmed will not be extended into production." },
    ],
    calls: [
      { callId: "GONG-902", callDate: "2026-06-16", title: "Phase 2 pilot exit criteria review", participants: "Dana Whitfield (VP Data Engineering), Marcus Rho (Director of Platform Engineering), Ines Duarte (Staff Data Engineer), Yusuf Abdel-Rahman", transcript: "Walked the six phase 2 exit criteria one by one. Four pass: schema evolution, backfill of 14 months of history, RBAC mapping to their Okta groups, and dbt model parity against their existing warehouse. Two fail: zero row loss across a controlled Postgres failover, and p99 under 500ms at 40k events per second on the wide-fact workload. Dana said she will not take the expansion to Hal Brenner until both are green, and asked for a tuned configuration and a soak test plan before the end of the month. She also set the outside date: the streaming tier has to be in production before their November peak trading window, because the current nightly pipeline will not carry that volume and they will not run a cutover during peak.", committedNextStep: "Vantage to deliver a tuned ingestion configuration and a 72 hour soak test plan by 2026-06-26", nextStepStatus: "delivered" },
      { callId: "GONG-917", callDate: "2026-07-02", title: "Security architecture review", participants: "Priya Nandakumar (Principal Security Architect), Marcus Rho (Director of Platform Engineering), Yusuf Abdel-Rahman", transcript: "Priya took us through three items: customer-managed key enforcement on staged objects, data residency confirmation that no pilot data leaves eu-central-1, and SOC2 CC6.1 evidence for our own key rotation. She was constructive and said none of the three are unusual for a platform at this stage. She asked for a key management design document and a written residency attestation, and confirmed her team would verify against the pen test finding once received.", committedNextStep: "Vantage to supply the key management design doc and residency attestation by 2026-07-10", nextStepStatus: "delivered" },
      { callId: "GONG-934", callDate: "2026-07-14", title: "Platform team check-in", participants: "Marcus Rho (Director of Platform Engineering), Yusuf Abdel-Rahman", transcript: "Marcus said his team has started a parallel technical evaluation and that an internal build on their existing Kafka and Flink stack is on the table as a serious option. He was direct that the soak test abort and the unresolved failover behaviour are what put the build option back on the table, and he will not agree a re-run date until there is a root cause for the backpressure. Dana Whitfield did not attend and has not responded to three meeting invitations since 2026-06-26.", committedNextStep: null, nextStepStatus: "none" },
    ],
    usage: [
...

  {
    ...
    tickets: [
      { createdAt: "2026-06-23", subject: "Which connectors are available in the Frankfurt region", priority: "P2", status: "resolved", slaBreached: false, body: "Lena asked for the connector inventory in the EU region ahead of the EMEA plant cutover. Confirmed 14 of 19 connectors are generally available in Frankfurt. The SAP and Snowflake CDC connectors remain us-east only, with no committed EU date. Answer provided in writing." },
      { createdAt: "2026-06-30", subject: "Azure Private Link support for the ingest endpoint", priority: "P2", status: "open", slaBreached: false, body: "Atlas requires private ingest in both target Azure regions and cannot route analytics traffic over public endpoints under their new network standard. AWS PrivateLink is GA today. Azure Private Link is on the roadmap without a committed date. Ticket held open pending the architecture review." },
      { createdAt: "2026-07-07", subject: "p99 query latency rose from 480ms to 1.9s on the plant telemetry workspace", priority: "P1", status: "resolved", slaBreached: false, body: "Investigated a sharp p99 regression on the plant telemetry workspace. Traced to a cross-cloud round trip after Atlas relocated the EMEA plant telemetry source into Azure North Europe as a pre-migration pilot, while the workspace stayed in us-east. Advised colocating the read path. p99 returned to 610ms once Atlas moved the read replica. No platform-side fault found." },
      { createdAt: "2026-07-14", subject: "Data residency: confirm at-rest and in-transit boundaries for EU plant data", priority: "P2", status: "open", slaBreached: false, body: "Anders requested written confirmation that EU plant telemetry never leaves the EU region, covering primary storage, backups, and any support engineer access path. Draft documentation prepared, held for legal review on the Vantage side before it can be sent." },
    ],
    calls: [
      { callId: "GONG-503", callDate: "2026-06-25", title: "Azure migration architecture review", participants: "Karen Ives, Lena Brandt, Yusuf Abdel-Rahman", transcript: "Atlas is moving its analytics estate off AWS and onto Azure across the next three quarters, starting with the EMEA plants in Q4. Lena walked the target topology: Azure North Europe for EU workloads, Azure East US for everything else, private networking mandatory in both. The current us-east deployment sits outside that plan and Karen asked directly what a supported Azure path looks like and what it costs her team to get there.", committedNextStep: "Vantage to produce an Azure deployment options paper by 2026-07-06", nextStepStatus: "delivered" },
      { callId: "GONG-517", callDate: "2026-07-02", title: "Security and residency deep dive", participants: "Anders Holm, Karen Ives, Yusuf Abdel-Rahman", transcript: "Anders is working to a new group policy that keeps EU plant telemetry inside the EU, including backups and any vendor support access. He asked for the key custody model and whether customer managed keys are supported in the EU region. He also pulled three open items from last year's pen test summary and asked for them to be restated against the current architecture. He took no position on the renewal itself.", committedNextStep: "Vantage to send residency and key custody documentation by 2026-07-13", nextStepStatus: "missed" },
      { callId: "GONG-528", callDate: "2026-07-16", title: "Renewal and roadmap checkpoint", participants: "Bill Tran, Karen Ives, Yusuf Abdel-Rahman", transcript: "Bill said the platform has been stable and that reliability is not what he is weighing. His question is whether the architecture follows Atlas onto Azure inside the migration window his team has already committed to the board. Karen mentioned her platform group is standing up an internal lakehouse on Azure for raw plant telemetry. Nobody put a decision date on the renewal beyond the existing August close.", committedNextStep: "Azure reference architecture review on 2026-07-23", nextStepStatus: "none" },
    ],
    usage: [
