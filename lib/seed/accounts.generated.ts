...
export const GENERATED_ACCOUNTS: AccountSeed[] = [
  {
    ...
    tickets: [
      { createdAt: "2026-06-08", subject: "Oracle CDC connector drops LOB columns on resync", priority: "P2", status: "resolved", slaBreached: false, body: "Change-data-capture from the legacy Oracle order store silently omitted CLOB and BLOB columns whenever a resync was triggered. Reproduced on the 11.2 source, patched the connector, and the customer re-ran the affected partitions cleanly." },
      { createdAt: "2026-06-22", subject: "Historical backfill of 4.1B order rows exceeds the weekend batch window", priority: "P2", status: "resolved", slaBreached: false, body: "The full three-year order history did not complete inside the 52 hour weekend window. Split the load into 14 partition-aligned chunks and raised parallelism to 24 writers, which brought the run to 31 hours." },
      { createdAt: "2026-07-06", subject: "Dual-run reconciliation mismatch on the inventory fact table", priority: "P2", status: "open", slaBreached: false, body: "During dual-run, roughly 0.4 percent of inventory rows differ between the legacy warehouse and Vantage, concentrated in adjustments posted after 22:00 local. Investigating a timezone handling difference in the legacy extract. Still open." },
    ],
    calls: [
      { callId: "GONG-944", callDate: "2026-06-11", title: "Phase 2 migration architecture review", participants: "Helena Voss, Marcus Lee, Dev Sundaram, Yusuf Abdel-Rahman", transcript: "Walked the Phase 2 target architecture covering orders, inventory and pricing, with a dual-run period of four weeks per domain before the legacy extracts are retired. Marcus said his platform group is two engineers, both of whom are also carrying the point-of-sale replatform through Q3. Dev raised the question of whether the reconciliation layer should live in Vantage or stay in their existing control framework, and asked for a written runbook before committing.", committedNextStep: "Vantage to deliver the Phase 2 cutover runbook by 2026-06-26", nextStepStatus: "delivered" },
      { callId: "GONG-961", callDate: "2026-07-09", title: "Dual-run reconciliation checkpoint", participants: "Helena Voss, Marcus Lee, Yusuf Abdel-Rahman", transcript: "Helena confirmed the Phase 2 cutover date has moved from 2026-08-03 to 2026-09-14 and asked us to re-baseline the runbook against the new date. Marcus walked through the open inventory reconciliation gap on the dual-run and said he is comfortable it is an extract-side timezone issue rather than an ingestion defect. Dev Sundaram was invited and did not attend, and no apology or reason was logged. Helena remains positive on Phase 1 and cited the orders domain running for six weeks without an incident.", committedNextStep: "Vantage to re-baseline the Phase 2 cutover runbook against the September date by 2026-07-24", nextStepStatus: "none" },
    ],
    usage: [
  ...
  },
  {
    ...
    tickets: [
      { createdAt: "2026-06-15", subject: "Ingest backpressure above 640k events per second on the SCADA topic", priority: "P1", status: "resolved", slaBreached: true, body: "Sustained SCADA ingest above roughly 640k events per second triggered consumer lag and backpressure into their collectors, with 90 minutes of delayed telemetry before mitigation. Rebalanced partitions from 96 to 240 and the cluster now holds a sustained 680k events per second. First response missed the P1 target by four hours." },
      { createdAt: "2026-06-29", subject: "p99 latency on the 90 day telemetry rollup exceeds 4 seconds", priority: "P2", status: "open", slaBreached: false, body: "The 90 day substation rollup returns at p95 1.4 seconds and p99 4.6 seconds against Granite Peak's internal 2 second target. Reproduced on their cluster. Candidate fixes under review are a pre-aggregated daily rollup and a change to the partition pruning strategy. Still open." },
      { createdAt: "2026-07-12", subject: "Request: benchmark harness and sizing guidance for 1.2M events per second", priority: "P3", status: "resolved", slaBreached: false, body: "Rob asked for the replay harness and a documented sizing model so his team can run the load test on their own infrastructure. Shipped the harness, the cluster sizing worksheet and the recommended broker and writer counts for the 1.2M events per second target." },
    ],
    calls: [
      { callId: "GONG-948", callDate: "2026-06-18", title: "Throughput ceiling review", participants: "Rob Farrow, Nadia Kessler, Yusuf Abdel-Rahman", transcript: "Rob presented the measurements from the June 15 incident showing a sustained ceiling around 680k events per second on the current cluster shape before consumer lag builds. Granite Peak's forecast for the winter peak is 1.1M events per second across 34 substations, with a short duration burst target of 1.2M. Nadia said the expansion case rests on demonstrating that headroom rather than on price. She referred to an internal capacity study her team completed in May and said its conclusions are what the operations technology committee will weigh, but that she cannot circulate it outside Granite Peak. Agreed we would produce a sizing model for the 1.2M target.", committedNextStep: "Vantage to deliver a sizing model for 1.2M events per second by 2026-07-02", nextStepStatus: "delivered" },
      { callId: "GONG-966", callDate: "2026-07-14", title: "Load test planning", participants: "Rob Farrow, Craig Bettis, Yusuf Abdel-Rahman", transcript: "Agreed a joint load test on 2026-07-29 replaying a 90 minute winter-peak window of captured SCADA traffic at two times speed, with pass criteria of 1.2M events per second sustained for 30 minutes at under 5 seconds end to end lag. Craig confirmed the Tier 4 purchase decision follows the load test result and that budget is already provisioned in the operations technology plan. Rob asked that the open rollup latency work stay on a separate track so it does not gate the test.", committedNextStep: "Joint load test to 1.2M events per second on 2026-07-29", nextStepStatus: "none" },
    ],
    usage: [
